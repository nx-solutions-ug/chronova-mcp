...
- Builds a single `McpServer({ name: "chronova-mcp", version: VERSION })` (where `VERSION` is loaded from `package.json` by `src/version.ts`) and connects the stdio transport.
...
- `GET /health` returns `{ status: "ok", version: VERSION }` — where `VERSION` comes from `package.json` via `src/version.ts` — useful for liveness probes in containers.
