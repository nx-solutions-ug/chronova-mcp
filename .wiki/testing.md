...
- **`server.test.ts`** — `/health` returns `{ status: "ok", version: VERSION }` (read from `package.json`); `initialize` returns `serverInfo.name = "chronova-mcp"` and the same version; `tools/list` returns exactly 4 tools with the expected sorted names; every tool has `annotations.readOnlyHint: true` and an `inputSchema.type = "object"`; an unknown `Mcp-Session-Id` yields HTTP 400 with "Invalid or expired session ID".
...
Tests are run on Node 25 in CI (see `.github/workflows/test.yml`). No coverage threshold or separate e2e suite is configured.