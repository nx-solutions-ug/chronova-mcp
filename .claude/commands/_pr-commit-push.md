...
4. Run the available quality gates (`bun run lint`, `bun run type-check`) if they are relevant and fast. Skip `bun run test` if it needs a database or external services.
